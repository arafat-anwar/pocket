<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table = $('.datatable-serverside').DataTable({
            ...
            initComplete: function (settings, json) {
                ...
                table.columns().every(function () {
                    ...
                    var input = $('<input type="text" style="width: '+(count == 1 ? '85px' : '100%')+' !important;" class="form-control text-center"  placeholder="'+translate(header)+'" title="'+translate(header)+'"/><span style="display: none">'+translate(header)+'</span>')
        ...
        $('.buttons-colvis').addClass('btn-sm').html('<i class="fas fa-eye-slash"></i>').attr('title', "Column Visibility");
        ...
        $('.buttons-colvis').click(function(event) {
            if($('.dt-button-collection').find('.dropdown-menu').find('.colvis-search-panel').length == 0){
                var page_direction = "{{ direction() }}";
                if(page_direction == 'rtl'){
                    $('.dt-button-collection').find('.dropdown-menu').css('min-width', '275px');
                    $('.dt-button-collection').find('.dropdown-menu').prepend('<div class="colvis-search-panel" style="width: 100%;"><div style="float: left;clear: right"><input type="text" class="form-control" placeholder="Search Columns..." style="width: 97%;margin: 5px" onkeyup="searchColumnVisibility($(this))" onchange="searchColumnVisibility($(this))"></div><div style="width: 25px;float: left;clear: right"><label style="cursor: pointer;margin-bottom: 0px"><input type="checkbox" id="check-all-columns" onclick="checkAllColumns()" style="margin-top:17px;pointer-events: none !important;margin-left: 10px;transform: scale(1.5, 1.5)"></label></div></div>');
                }else{
                    $('.dt-button-collection').find('.dropdown-menu').css('min-width', '250px');
                    $('.dt-button-collection').find('.dropdown-menu').prepend('<div class="colvis-search-panel" style="width: 100%;"><div style="width: 25px;float: left;clear: right"><label style="cursor: pointer;margin-bottom: 0px"><input type="checkbox" id="check-all-columns" onclick="checkAllColumns()" style="margin-top:17px;pointer-events: none !important;margin-left: 10px;transform: scale(1.5, 1.5)"></label></div><div style="float: left;clear: right"><input type="text" class="form-control" placeholder="Search Columns..." style="width: 97%;margin: 5px" onkeyup="searchColumnVisibility($(this))" onchange="searchColumnVisibility($(this))"></div></div>');
                }
    ...
    function updateColumnVisibility(ajax_call = true) {
        ...
        if(ajax_call){
            $.ajax({
                url: "{{ url('setups/update-user-column-visibilities') }}",
                type: 'POST',
